# KOA Rest API with mongodb driver
Rest API based on node js, KOA and mongo driver with JSON Schema validation (which is native for mongo)
...
#### 1. Copy the settings.js file from keepass:
```
export default {
...
## Creating new lib service:

```
yarn createLib [libName]
```

Mostly the libName is the name of the new collection. It will create corresponding file names.
