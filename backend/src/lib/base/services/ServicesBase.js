...
class ServicesBase {
  ...
  async executeBulk(fullArray, response = []) {
    if (!fullArray.length) return response;
    const toExecute = fullArray.splice(0, 1000);
    if (toExecute.length) response.push(await this.DB.bulkWrite(toExecute));
    return this.executeBulk(fullArray, response);
  }
