    ...
    if (!valid) {
      ...
      validate.errors?.forEach(validateError => {
        ...
        if (!instancePath && params.missingProperty) {
          ...
          errorPath = params.missingProperty;
        }
      ...
      if (!returnErrors) {
        return ctx.modS.responses.createErrorResponse(ctx, ctx.modS.responses.CustomErrors.BAD_REQUEST, {
          data: { errors, detailedErrors },
          realError: JSON.stringify(validate.errors),
        });
      }
      return errors;
    }
